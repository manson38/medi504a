...
diab1 <- diab %>% 
  ...
  mutate(decade = factor(case_when(Age < 20 ~ "10",
                                   Age < 30 ~ "20",
                                   Age < 40 ~ "30",
                                   Age < 50 ~ "40",
                                   Age < 60 ~ "50",
...
diab1 %>% 
  ...
  geom_boxplot(width = 0.15, show.legend = FALSE, alpha = 0.8) + 
...
# Plot
diab1 %>% 
  ggplot(aes(x = Pregnancies, y = decade)) +
...
p1_3_2 <- diab1 %>% 
  ggplot(aes(x = decade, y = Pregnancies, fill = Outcome)) + 
